(function() {
	...
	RSSParser.prototype.parse = function(callback) {
		...
		try {
			...
			this.posts.forEach(function(post) {
				if (!isNaN(post.published_from_feed) && typeof post.published_from_feed === "number") {
    ...
    var link = this.parseLink(rootElement);
		...
		var titleEl = rootElement.find('title,rss\\:title').first();
		this.data.title = RSSParser.trimChars(titleEl.text());

		var posts = rootElement.find('item,rss\\:item');
		for ( var i = 0, post; (post = posts[i]); i++ ) {
			post = this.$(post);

			var titleElement = post.find('title,rss\\:title').first();
	...
	RSSParser.prototype.parsePostLink = function(links) {
		...
		function pointsForObject(a) {
			...
			var points = -1;
			if (rel == "self" && (type == "text/html" || type === "application/html")) {
				points += 10;
			}
			if (rel == "alternate") {
				points += 1;
    ...
    const dateSelectors = 'published, updated, pubDate, dc\\:date, date, created, issued';
    var datePublished = post.children(dateSelectors).first();
    if (!datePublished) {
      datePublished = post.find(dateSelectors).first();
    }
		return this.parseDate(datePublished && datePublished.text())
	}

	RSSParser.prototype.parseDate = function(dateStr) {
		var date;
		if (dateStr) {
