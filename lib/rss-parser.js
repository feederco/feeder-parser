...
var RSSParser = Class.extend({
	...
	setResult: function(text, callback) {
		...
		try {
			...
			rootElement = this.$.root().children().first();
		} catch (e) {
		 	rootElement = false;
		}
	...
	doParse: function(callback) {
		...
		// Test for RSS
		var type = false;
		var rootTag = rootElement[0].name.toLowerCase();

		if ( rootTag == 'rss' )
			type = 'rss';
		else if ( rootTag == 'feed' )
			type = 'atom';
		else if ( rootTag == 'rdf' || rootTag == 'rdf:rdf' )
			type = 'rss';
	...
	parseRSSResponse: function(rootElement) {
		...
		for ( var i = 0, post; (post = posts[i]) && i < 50; i++ ) {
			...
			if ( descriptionElement.text() ) {
				summary = descriptionElement.text();
			}
	...
	getDate: function(post) {
		var datePublished = post.find('published, updated, pubDate, dc\\:date, date');
...
RSSParser.matchTag = function(el, tagName) {
	do {
		if ( el.name.toLowerCase() === tagName )
			return el;
	} while ( (el = el.parent) && el.name );
	return false;
}
